const { getConnection, runQueryValues, mycoursesSyntax, existingSignup, courseQuery, existingEnrollment } = require('../model/dbPool');
...
async function mycourses(req, res) {
  ...
  try {
    ...
    const userExists = await runQueryValues(connection, existingSignup, [course.user_id]);
    ...
    const existingEnrollmentCheck = await runQueryValues(connection, existingEnrollment, [course.user_id, course.course_id]);

    if (existingEnrollmentCheck.length > 0) {
      connection.release();
...
//         res.status(failed).json({ message: 'You have already added this course.' });
//     }else {
//       res.status(500).json({ error: 'An unknown error occurred while adding the course.' });
//     }
